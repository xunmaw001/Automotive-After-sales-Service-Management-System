    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑出入库订单</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑出入库订单</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">出入库订单信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>类型</label>
                                        <select id="orderTypesSelect" name="orderTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("inOutOrder/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addinOutOrder');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationOrdertypesSelect(){
            var orderTypesSelect = document.getElementById('orderTypesSelect');
            if(orderTypesSelect != null && orderTypesOptions != null  && orderTypesOptions.length > 0 ){
                for (var i = 0; i < orderTypesOptions.length; i++) {
                    orderTypesSelect.add(new Option(orderTypesOptions[i].indexName,orderTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(orderTypesSelect != null && orderTypesOptions != null  && orderTypesOptions.length > 0 ) {
            for (var i = 0; i < orderTypesOptions.length; i++) {
                if (orderTypesOptions[i].codeIndex == ruleForm.orderTypes) {//下拉框value对比,如果一致就赋值汉字
                        orderTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    // 获取当前详情
    function getDetails() {
        var addinOutOrder = window.sessionStorage.getItem("addinOutOrder");
        if (addinOutOrder != null && addinOutOrder != "" && addinOutOrder != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("inOutOrder/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationOrdertypesSelect();
            <!--  初始化级联表的下拉框  -->
